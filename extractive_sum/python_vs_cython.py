...
'''
This code visualizes the relationship between the number of rows and the time taken to process them for single-processing, multi-processing, and Cython modes.
'''

# File paths
TIME_FILE = 'time_vs_rows.txt'
MP_TIME_FILE = 'time_vs_rows_mp.txt'
CYTHON_TIME_FILE = 'time_vs_rows_cython.txt'
...
def main():
    ...
    cython_data = read_data(CYTHON_TIME_FILE)
    if not cython_data:
        print(f"No data found in {CYTHON_TIME_FILE}. Exiting.")
        return

    # Parse the data
    single_rows, single_times = zip(*single_data)
    multi_rows, multi_times = zip(*multi_data)
    cython_rows, cython_times = zip(*cython_data)
    ...
    cython_coef = fit_quadratic(cython_rows, cython_times)
    store_coefficients(CYTHON_COEF_FILE, cython_coef)

    # Generate fitted values
    single_fit = np.polyval(single_coef, single_rows)
    multi_fit = np.polyval(multi_coef, multi_rows)
    cython_fit = np.polyval(cython_coef, cython_rows)

    # Read multiprocessing configuration
    config = read_config(MP_CONFIG_FILE)

    # Plot the data
    plot_data(single_rows, single_times, single_fit, multi_rows, multi_times, multi_fit, cython_rows, cython_times, cython_fit, config)
